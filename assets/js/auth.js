...
function seedUserIfEmpty() {
    ...
    if (!raw) {
        const seed = [
            { name: 'Admin', email: 'admin@gmail.cl', pass: 'admin123', role: 'admin' },
...
function registerUser({ name, email, pass }) {
    ...
    if (findUserByEmail(email)) {
        return { ok: false, code: 'already_exists', message: 'El correo ya esta registrado.' };
    }
...
function loginUser(email, pass) {
    if(!email?.trim() || !pass?.trim()) {
        return {ok: false, code: 'missing_fields', message: 'Correo y contraseña son obligatorias'}
    }
    if (!emailDomainOk(email)) {
    ...
    if(!user) {
        return {ok: false, code: 'not_found', message: 'Este correo no esta registrado.'}
    }
    if (user.pass !== pass) {
       return {ok: false, code:'bad_credentials', message: 'Usuario o contraseña incorrectas'}
    }
